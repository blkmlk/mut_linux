...
#ifdef CONFIG_IP_MUT
static int ipmr_mut_add_dst(struct mr_table *mrt, struct mfcctl *mfc)
{
	struct mfc_cache *c;
	struct mut_dst *new_m_dst, *i;

	c = ipmr_cache_find(mrt, mfc->mfcc_origin.s_addr, mfc->mfcc_mcastgrp.s_addr);
	...
	if (ipv4_is_multicast(mfc->ip.s_addr) || ipv4_is_lbcast(mfc->ip.s_addr)) {
		return -EINVAL;
	}

	list_for_each_entry(i, &c->mut_list, list) {
		if (mfc->ip.s_addr == i->ip) {
			return 0;
		}
	}

	new_m_dst->ip = mfc->ip.s_addr;

	list_add(&new_m_dst->list, &c->mut_list);

	return 0;
}

static int ipmr_mut_del_dst(struct mr_table *mrt, struct mfcctl *mfc) {
	struct mfc_cache *c;
	struct mut_dst *d;
...
int ip_mroute_setsockopt(struct sock *sk, int optname, char __user *optval, unsigned int optlen)
{
	...
	case MRT_MUT_DEL_DST:
		if (optlen != sizeof(struct mfcctl))
			return -EINVAL;
		if (copy_from_user(&mfc, optval, sizeof(struct mfcctl)))
			return -EFAULT;

		rtnl_lock();
		if (optname == MRT_MUT_DEL_DST)
			ret = ipmr_mut_del_dst(mrt, &mfc);
		else
			ret = ipmr_mut_add_dst(mrt, &mfc);
		rtnl_unlock();
		return ret;
#endif
		/*
...
forward:
	...
	for (ct = cache->mfc_un.res.maxvif - 1;
		...
		list_for_each_entry(m_dst, &cache->mut_list, list) {
			struct sk_buff *skb2 = skb_clone(skb, GFP_ATOMIC);
	...
	if (v == SEQ_START_TOKEN) {
		...
		if (it->cache != &mrt->mfc_unres_queue) {
			...
			for (n = mfc->mfc_un.res.minvif;
				...
				struct mut_dst *m_dst;
				list_for_each_entry(m_dst, &mfc->mut_list, list) {
					seq_printf(seq, "%08pI4\n", &m_dst->ip);
				}
			} 
#endif
		} else {
